...
function cargarClientes() {
    ...
    for (let i = 0; i < clientes.length; i++) {
        cadena += `<option value="${clientes[i].nombre}">${
...
function cargarCarrito() {
    ...
    for (let i = 0; i < carrito.length; i++) {
        cadena += `<tr>
            ...
            <td>
                <input type="number" onchange="cambiaCantidad(${i}, this)" value="${
            carrito[i].cantidad
        }" class="form" placeholder="cantidad">
            </td>
...
function cambiaCantidad(posicion, elemento) {
    ...
    carrito[posicion].cantidad =
        elemento.value != null && elemento.value != '' ? elemento.value : 1;
...
function calcularTotal() {
    ...
    document.getElementById('totalNeto').innerText =
...
function registrarVenta() {
    ...
    for (let i = 0; i < carrito.length; i++) {
        for (let j = 0; j < losProductos.length; j++) {
            if (losProductos[j].nombre == carrito[i].producto) {
...
// Function to load product search
function buscarProducto() {
    ...
    if (buscador.trim() == '' || buscador.trim() == null) {
        ...
        for (let i = 0; i < productos.length; i++) {
            var texto = productos[i].nombre.toLowerCase();
...
function eliminarVenta(posicion) {
    ...
    Swal.fire({
        title: '¿Está seguro?',
        text: 'La venta se eliminará, y se devolverá el stock!',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            for (let i = 0; i < laVenta.detalles.length; i++) {
                for (let j = 0; j < losProductos.length; j++) {
                    if (
                        losProductos[j].nombre == laVenta.detalles[i].producto
                    ) {
                        losProductos[j].stock =
                            parseInt(losProductos[j].stock) +
                            parseInt(laVenta.detalles[i].cantidad);
                    }
                }
            }
            localStorage.setItem('productos', JSON.stringify(losProductos));
...
function verVenta(posicion) {
    ...
    window.location.href = 'verVenta.html';
}
...
function mostrarVenta() {
    ...
    document.getElementById('totalNeto').innerText =
        parseFloat(laVenta.total) - parseFloat(laVenta.descuento);
    ...
    for (let i = 0; i < laVenta.detalles.length; i++) {
        var subtotal =
            parseFloat(laVenta.detalles[i].precio) *
            parseFloat(laVenta.detalles[i].cantidad);
        cadena += `
            <tr>
